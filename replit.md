...
-   **Role-Scoped Admin Tabs**: The Admin page's "Staff Management" tab is rendered only for `super_admin`; leaders default to the "My Team" tab and never see Staff Management. From My Team, leaders use the "Add Team Member" dialog to create a basic user by name + email (auto-scoped to their primary SPU, with optional sub-unit selection); the server validates that leaders can only create users in SPUs they manage and that the chosen sub-unit belongs to that SPU. Created users automatically get a `leader_basic_assignment` so they appear in the leader's My Team list and remain visible in super_admin Staff Management.
